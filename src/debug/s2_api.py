...
class SemanticScholarKit:
    ...
    def _sync_get_papers(self, paper_ids: List[str]) -> List[Dict]:
        """Synchronous helper for the actual API call, used with asyncio.to_thread."""
        try:
            # Pass fields to the underlying library call
            batch_results: List[Paper] = self.scholar.get_papers(paper_ids=paper_ids)
            # Process results immediately after the blocking call finishes in the thread
            return [item._data for item in batch_results if hasattr(item, '_data')]
        except Exception as e:
            # Log error within the thread, it will be raised or caught by gather
            logger.error(f"Error in _sync_get_papers for batch (first 5 IDs: {paper_ids[:5]}...): {e}")
            # Option 1: Return empty list on error in batch
            return []
            # Option 2: Re-raise the exception to be caught by asyncio.gather
            # raise e

    async def search_paper_by_ids(
        self,
        id_list: List[str]
    ) -> List[Dict]:
        ...
        if id_cnt > 0:
            ...
            logger.info(f"Fetching {id_cnt} papers by ID in {batch_cnt} batches.")

            tasks = []
            for batch in batches:
                # Create a task for each batch using asyncio.to_thread
                tasks.append(asyncio.to_thread(self._sync_get_papers, batch))

            # Run all batch tasks concurrently in threads managed by asyncio
    ...
    async def search_author_by_ids(
        self,
        author_ids: List[str],
        ...
        if with_abstract and author_metadata:
            ...
            if papers_missing_abstract_ids:
                ...
                paper_metadata_with_abstracts = await self.search_paper_by_ids(
    ...
    async def search_paper_by_keywords(
        self,
        query: str,
        year: str = None,
    ...
    async def get_s2_cited_papers( # Corresponds to get_paper_references
        self,
        paper_id: str,
        limit: int = 100,
        with_abstract: Optional[bool] = False
        ...
        if with_abstract and refs_metadata:
            ...
            if papers_missing_abstract_ids:
                ...
                paper_metadata_with_abstracts = await self.search_paper_by_ids(id_list=list(papers_missing_abstract_ids))
                ref_abstracts = {
                    item['paperId']: item['abstract']
                    for item in paper_metadata_with_abstracts
                    if item.get('paperId') and item.get('abstract')
    ...
    async def get_s2_citing_papers( # Corresponds to get_paper_citations
        self,
        paper_id: str,
        limit: int = 100,
        with_abstract: Optional[bool] = False
    ...
    async def get_s2_recommended_papers(
        self,
