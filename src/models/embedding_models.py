...
# gemini embedding - async
async def _gemini_batch_embedding_async(api_key, model_name, texts_btch, semaphore):
...
async def gemini_embedding_async(api_key, model_name, texts: List[str], n_concurrent: int) -> np.ndarray:
    ...
    for i in range(n):
        texts_btch = texts[i*100: i*100+100]
        task = asyncio.create_task(_gemini_batch_embedding_async(api_key, model_name, texts_btch, semaphore))
        tasks.append(task)
