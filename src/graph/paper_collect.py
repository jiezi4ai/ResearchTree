...
S2_REQUEST_INTERVAL = 3  # Seconds between requests
...
class PaperSearch:
    ...
    def __init__(
        ...
        """
        self.s2 = SemanticScholarKit()  # semantic scholar for data reference
        # seed papers 
    ...
    async def collect(
            ...
            reference_limit: Optional[int]=100,
        ...
        if with_reference and seed_paper_dois:
            ...
            for paper_doi in seed_paper_dois:
                tasks.append(self.get_cited_papers(paper_doi, reference_limit, from_dt, to_dt, fields_of_study))
                tasks.append(self.get_citing_papers(paper_doi, reference_limit, from_dt, to_dt, fields_of_study))

        # --- Task Prep: Recommendations ---
        if with_recommend and seed_paper_dois:
             logging.info("Preparing recommendations task.")
             tasks.append(self.get_recommend_papers(seed_paper_dois, recommend_limit, from_dt, to_dt, fields_of_study))
        ...
        if with_expanded_search and seed_paper_dois:
            async def _handle_expanded_search():
                ...
                if keywords_topics_json:
                    ...
                    related_papers_items = await self.get_related_papers(keywords_topics_json, search_limit, from_dt, to_dt, fields_of_study)
                else:
