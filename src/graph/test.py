...
class PaperSearch:
    ...
    async def initial_paper_query(
        ...
        if self.seed_paper_dois:
            async def _fetch_by_dois():
                ...
                s2_paper_metadata = await self.s2.async_search_paper_by_ids(id_list=self.seed_paper_dois)
        ...
        if self.seed_paper_titles:
            async def _fetch_by_title(title):
                ...
                s2_paper_metadata = await self.s2.async_search_paper_by_keywords(query=title, fields_of_study=fields_of_study, limit=limit)
        ...
        if self.research_topic:
            async def _fetch_by_topic():
                ...
                s2_paper_metadata = await self.s2.async_search_paper_by_keywords(query=self.research_topic, fields_of_study=fields_of_study, limit=limit)
    ...
    async def get_related_papers(
        ...
        async def _fetch_related_for_query(query):
            ...
            s2_paper_metadata = await self._rate_limited_s2_call(
                self.s2.async_search_paper_by_keywords(query, fielfields_of_studyds=fields_of_study, limit=limit)
            )
            logging.info(f"Processing {len(s2_paper_metadata)} related papers for query '{query[:50]}...'")
            # Assuming process_related_metadata is similar to process_paper_metadata
            s2_papermeta_json = process_related_metadata( # Or use process_paper_metadata if structure is identical
    ...
    async def cal_semantic_similarity(self, paper_nodes_json: List[Dict]) -> List[Dict]:
        ...
        for i in range(rows):
            # publish_dt_i = publish_dt_ref.get(ids[i]) # Not needed if start/end determined by ID comparison
            for j in range(i + 1, cols): # Iterate only upper triangle (j > i)
                sim = sim_matrix[i, j]
                # publish_dt_j = publish_dt_ref.get(ids[j]) # Not needed

                # Determine start/end node consistently (e.g., lexicographically)
                # to avoid duplicates A->B and B->A being added separately
                id_i = ids[i]
                id_j = ids[j]
                if id_i < id_j:
                     start_node_id = id_i
                ...
                if pair_tuple not in added_pairs:
                    edge = {
                        ...
                        "properties": {
                            ...
                            # 'texts':(paper_info_ref.get(start_node_id), paper_info_ref.get(end_node_id)) # Optional: might be too large
                         }
                     }
...
async def main():
    ...
    searcher = PaperSearch(
        ...
        seed_paper_dois=seed_dois, # Example DOI
    ...
    await searcher.init_collect(limit=100, from_dt="2015-01-01", to_dt="2025-04-01")
    ...
    await searcher.collect(
        ...
        with_recommend=True,
        with_expanded_search=False, # Set to True to test LLM topic generation
        add_semantic_similarity=True,
        similarity_threshold=0.75,
        limit=20, # Limit number of items per fetch step
        from_dt="2015-01-01",
        to_dt="2025-04-01"
    )
